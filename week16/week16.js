const forms = document.forms; // это для удобства я себе сделала
//Задание 1
...
function makeOne() {
	...
	paragraph.textContent = "найдено форм: " + allForms.length;
}
...
function makeTwo() {
	const form1 = forms[0];
	const form1Name = form1.getAttribute("name");
	console.log(form1Name);
	paragraphTwo.textContent = "значение атрибута 'name' первой формы: " + form1Name;
}
...
function makeThree() {
	...
	paragraphThree.textContent ="значение атрибута 'name' последней формы: " + lastFormName;
}
...
function makeFour() {
	...
	const str = formNames.join(', ');
	// console.log(str);
	paragraphFour.textContent = str;
}
...
function makeFive() {
	...
	console.log(form3.elements.length)
	paragraphFive.textContent = "количество элементов: " + form3.elements.length;
}
...
function makeSix() {
	...
	console.log(form2.elements.length)
	paragraphSix.textContent = "количество элементов: " + form2.elements.length;
}
...
function makeEight() {
	...
	for(let i = 0; i < form1Elements.length; i++){
		const element = form1Elements[i];
		const name = element.getAttribute("name");
		// console.log(name);
		names1.push(name);
	}
// console.log(names1)
paragraphEight.textContent = "все элементы первой формы называются: " + names1;
}
...
function makeNine() {
	...
	const form3Elements = form3.elements;
	let names3 = '';
	for(let i = 0; i < form3Elements.length; i++){
		const element = form3Elements[i];
		const name = element.getAttribute("name");
		names3 += `${name}, `
	}
	paragraphNine.textContent = names3;
}
...
function makeTen() {
	...
	const form4Elements = form4.elements;
	// console.log(form4Elements)
	const radioValue = form4[0].value;
...
function makeEleven() {
	...
	for(let i = 0; i < select1.length; i++){
		let options = select1[i]
		console.log(options)
		optionsValues += `${options.value}, `
	}
...
for (let i = 0; i < elements2.length; i++) {
    ...
    if (element.classList.contains('form__checkbox')) {
		const id = element.getAttribute("id");
		checkbox2 += `${id}, `;
	}
}
// console.log(checkbox2)
paragraphTwelve.textContent = checkbox2; // если можно, я бы хотела посмотреть на менее громоздкий вариант этого задания
}
...
function checkButton(e) {
	e.preventDefault();
	const radio4 = document.querySelector('input[name="fourthName"]:checked');
	if (radio4) {
        paragraphThirteen.textContent = 'кнопка нажата';
    } else {
        paragraphThirteen.textContent = 'кнопка не нажата';
    }
} //тоже интересно как сделать по-другому
...
function makeSixteen() {
	const form2 = document.forms[1];
	const checkbox3 = form2.elements.checkboxThree;
...
const nameOne = name1.value;
const email1 = document.getElementById('firstEmail');
const emailOne = email1.value;
const formOne = document.forms.formOne;
formOne.addEventListener('submit', function (event) {
	event.preventDefault(); 
	formOne.reset();

if(emailOne.length === 0 && nameOne.length === 0){
	error1.textContent = "введите имя и почту!"
}
if(emailOne.length > 0 && nameOne.length === 0){
	error1.textContent = "введите имя!"
}
if(emailOne.length === 0 && nameOne.length > 0){
	error1.textContent = "введите почту!"
}})
...
formOne.addEventListener('submit', function (event) {
	...
	const emailOne = email1.value;
	const regexp = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
	if(regexp.test(emailOne)){
		console.log('все нормально :)');
	}else {
        email1.placeholder = 'введите заново'
    }
}); // это будет криво работать, если закомментировать почти всё сверху, если не закомментировать, работать не будет вообще
...
const form4 = document.forms[3];
const radio4 = form4.elements.fourthName;

document.querySelector('.b-23').onclick = function (event) {
	if (radio4.checked) {
		document.querySelector('.b-13').disabled = false;
	}
	else {
...
document.querySelector('.b-24').onclick = function (event) {
	event.preventDefault();
	inputs = document.querySelectorAll('.form__input');
	// console.log(inputs);
	inputs.forEach(function(item){
		item.setAttribute('placeholder', 'Введите данные');
	}
	)};
...
document.querySelector('.b-26').onclick = function (event) {
	...
	const inputName3 = document.getElementById('thirdName');
	const placeholderValue3 = inputName3.getAttribute('placeholder');
	result.textContent = placeholderValue3;
};
